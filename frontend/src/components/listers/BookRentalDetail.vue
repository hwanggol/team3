<template>
    <v-card outlined>
        <v-card-title>
            BookRental # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="RentalStartTime" v-model="item.rentalStartTime" :editMode="editMode" @change="change" />
            </div>
            <div>
                <String label="RentalEndTime" v-model="item.rentalEndTime" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/bookRentals/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
